# cave atlas working guide

Maintain a small static cave explorer. The accepted interface is a full-screen map, compact filters/list on the left, and a collapsible comparison on the right. Keep the visible name `cave atlas` lowercase and the results heading `Comparison` constant. Project direction belongs to snowball's founder, Nas Delevski.
...
- Keep operations and revival instructions in docs/OPERATIONS.md. Broader catalogue coverage and optional lazy-loaded 3D surveys are documented in docs/ROADMAP.md; neither is shipped by implication.
